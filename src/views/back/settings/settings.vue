<template>
  <div class="pc-settings column h-full">
      ...
      <div class="tab_content" v-show="!idx">
        ...
        <div class="headPortrait space-x-5">
          ...
          <ip-button class="px-5 h-8 text-sm" type="neutral" @click="setPassword">
            {{ $t("PCSettings.tabContent.headPortrait") }}
          </ip-button>
        </div>
        <!-- 用户信息 -->
        <div class="user_info">
    ...
    <!-- 验证密码弹出层 -->
    <el-dialog :title="$t('PCSettings.passwordDialog.title')" v-model="isDialog" width="22%" :before-close="closeIsDialog" class="passwordDialog">
          ...
          },
          {
            ...
            validator: (rule, val, callback) => {
              if (this.formData.newPassword === val) {
                ...
                callback(new Error(this.$t("PCSettings.form.passwordForm.rules[1]")))
              }
            },
            trigger: "change",
          },
      ...
      if (this.isForm) {
        this.$refs.Elform.validate(async (valid) => {
          ...
          try {
            ...
            this.$message({
              message: "If the password is successfully changed, you need to log in again",
              type: msg,
            })
